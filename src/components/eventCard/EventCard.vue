<template>
  <div :class="{ 'gallery-m': mobile(), 'gallery': !mobile() }">
    <div :class="{ 'card-m': mobile(), 'card': !mobile() }" v-if='value !== undefined' v-for="(x, index) in value" :key="x.eventId">
      <div :id=x.eventId>
        <router-link :to="{ name: 'EventDescription', params: { id: index }}">
          ...
          <div class="details-line">
            ...
            <div class="event-title-w">{{x.eventName}}</div><button class="details-button">Join</button>
          </div>
        </router-link>
<!--         <div>
          <div>{{x.eventCity}}</div>
